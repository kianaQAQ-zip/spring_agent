...
> ✅ **M9 已构建（2026-08-27）**：`eval/CostCalculator` + `ChatService.trackUsage`（流式 `chatResponse()` 读 Usage 汇总 token 成本）+ `eval/EvalRunner`（四维判分：关键词/意图/接地/忠实度）+ `eval/eval-set.json`（20 条）+ `common/TenantInterceptor` + `config/WebConfig`（X-Tenant-Id 注入）+ `README.md`（零改动复现）。测试：`CostCalculatorTest`(2)/`EvalRunnerTest`(3)。详见 `docs/M9_VERIFICATION.md`。
> 注：沙箱禁 TLS，`mvn test` 需你本机执行；沙箱内完成静态审查与代码交付。
