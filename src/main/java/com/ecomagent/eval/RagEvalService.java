...
public class RagEvalService {
     ...
     * @param cost          本轮成本估算
     */
    public void record(String conversationId, String query, int docCount,
                       int citationCount, int outOfRange, int answerTokens, double cost) {
        try {
            repository.insert(new RagEvalRecord(
                    conversationId, TenantContext.get(), PlatformContext.code(), query,
